...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO winners(WinnerID, AuctionID, UserID,BidID) VALUES (?, ?, ? ,?)");
    $stmt->bind_param("isss", $WinnerID, $AuctionID, $UserID,$BidID);
    // Set parameters and execute
    $WinnerID = $_POST['book_id'];
    $AuctionID = $_POST['ride_id'];
    $UserID = $_POST['passenger_id'];
    $BidID = $_POST['passengerid'];
...
<body>
    <center><h2>Winners Table</h2></center>
    <table border="3">
        <tr>
            <th>WinnerID</th>
            <th>AuctionID</th>
...
// Prepare SQL query to retrieve all winners
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $WinnerID = $row['WinnerID']; // Fetch the WinnerID
        echo "<tr>
            <td>" . $row['WinnerID'] . "</td>
            <td>" . $row['AuctionID'] . "</td>
            <td>" . $row['UserID'] . "</td>
            <td>" . $row['BidID'] . "</td>
            <td><a style='padding:4px' href='delete_watchlist.php?WinnerID=$WinnerID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_watchlist.php?WinnerID=$WinnerID'>Update</a></td> 
        </tr>";
    }
