...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <label for="WatchlistID">WatchlistID:</label>
    <input type="number" id="WatchlistID" name="WatchlistID" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO watchlist(WatchlistID, UserID, ItemID) VALUES (?, ?, ?)");
    $stmt->bind_param("iis", $WatchlistID, $UserID, $ItemID);
...
<body>
    <center><h2>Watchlist Table</h2></center>
    <table border="3">
        <tr>
            <th>WatchlistID</th>
            <th>UserID</th>
...
// Prepare SQL query to retrieve all watchlist
$sql = "SELECT * FROM watchlist";
$result = $connection->query($sql);

// Check if there are any watchlist
if ($result->num_rows > 0) {
    // Output data for each row
    while ($row = $result->fetch_assoc()) {
        $WatchlistID = $row['WatchlistID']; // Fetch the WatchlistID
        echo "<tr>
            <td>" . $row['WatchlistID'] . "</td>
            <td>" . $row['UserID'] . "</td>
            <td>" . $row['ItemID'] . "</td>
            <td><a style='padding:4px' href='delete_watchlist.php?WatchlistID=$WatchlistID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_watchlist.php?WatchlistID=$WatchlistID'>Update</a></td> 
        </tr>";
    }
